...
1. Run from the target working directory (which must already exist outside skill folders):
   ...
   `scripts/setup-agent-box.sh` relative to this skill folder. The helper asks
...
`agent-box-manage.sh` to the external working directory, create `.env` using the
fields in `scripts/setup-agent-box.sh` (`write_example`), generate/reuse
